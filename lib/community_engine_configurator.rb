...
module CommunityEngine
  ...
  class Configurator
    class << self
      ...
      #
      # require "#{RAILS_ROOT}/vendor/plugins/community_engine/lib/community_engine_configurator"
      #
      # before your "Rails::Initializer.run do |config|" block.
      # Then place this line:
      # 
      # CommunityEngine::Configurator.configure!(config)
      #
      ...
      def configure!(config, options = {})
        ...
        [
          ...
          ['htmlentities'],
          ['rake', {:version => '~> 0.8.3'}],
          # HAML 2.2.10 has support for rails_xss
          ['haml', {:version => '>= 2.2.10'}],
          ['ri_cal'],
          ['aws-s3', {:lib => "aws/s3"}]
